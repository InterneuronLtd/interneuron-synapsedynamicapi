...
namespace Interneuron.Synapse
{
    public class SynapseDynamicAPIClient
    {
        ...
        public string GetBaseViewList(string baseviewName, string orderBy = null, string limit = null, string offset = null, string filter = null)
        {
            ...
            string endpoint = $"GetBaseViewList?baseviewname={baseviewName}" +
                (string.IsNullOrWhiteSpace(orderBy) ? "" : $"&orderby={orderBy}") +
                (string.IsNullOrWhiteSpace(limit) ? "" : $"&limit={limit}") +
                (string.IsNullOrWhiteSpace(offset) ? "" : $"&offset={offset}") +
        ...
        public string GetBaseViewListByAttribute(string baseviewName, string synapseAttributeName, string attributeValue, string orderBy = null, string limit = null, string offset = null, string filter = null)
        {
            ...
            string endpoint = $"GetBaseViewListByAttribute?baseviewname={baseviewName}&synapseattributename={synapseAttributeName}&attributevalue{attributeValue}" +
                            (string.IsNullOrWhiteSpace(orderBy) ? "" : $"&orderby={orderBy}") +
                            (string.IsNullOrWhiteSpace(limit) ? "" : $"&limit={limit}") +
                            (string.IsNullOrWhiteSpace(offset) ? "" : $"&offset={offset}") +
                            (string.IsNullOrWhiteSpace(filter) ? "" : $"&filter={filter}");
